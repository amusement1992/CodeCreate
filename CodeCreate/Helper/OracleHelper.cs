...
namespace Prime.DBUtility
{
    ...
    public class OracleHelper
    {
        ...
        public static int ExecuteNonQuery(CommandType type, string sql, OracleParameter[] sps)
        {
            using (OracleConnection conn = new OracleConnection(connStr))
            {
                using (OracleCommand cmd = new OracleCommand(sql, conn))
                {
                    try
                    {
                        ...
                        {
                            //这个sql语句需要参数，做添加操作
                            foreach (OracleParameter sp in sps)
                            {
        ...
        /// <returns></returns>
        public static int ExecuteNonQuery(string sql, OracleParameter[] sps)
        {
        ...
        public static object ExecuteScalar(CommandType type, string sql, OracleParameter[] sps)
        {
            using (OracleConnection conn = new OracleConnection(connStr))
            {
                using (OracleCommand cmd = new OracleCommand(sql, conn))
                {
                    try
                    {
                        ...
                        if (sps != null)//添加参数
                        {
                            foreach (OracleParameter sp in sps)//这个sql语句需要参数，做添加操作
                            {
                                if (sp != null)//sps中的某个元素可能为空
                                {
                                    cmd.Parameters.Add(sp);
        ...
        /// <param name="sps">參數列表</param>
        /// <returns></returns>
        public static object ExecuteScalar(string sql, OracleParameter[] sps)
        {
        ...
        public static DataTable GetDataTable(CommandType type, string sql, OracleParameter[] sps)
        {
            using (OracleConnection conn = new OracleConnection(connStr))
            {
                using (OracleCommand cmd = new OracleCommand(sql, conn))
                {
                    try
                    {
                        cmd.CommandType = type;
                        conn.Open();
                        OracleDataAdapter sqlAdap = new OracleDataAdapter();
                        //添加参数
                        if (sps != null)
                        {
                            //这个sql语句需要参数，做添加操作
        ...
        /// <param name="sps">參數列表</param>
        /// <returns>DataTable</returns>
        public static DataTable GetDataTable(string sql, OracleParameter[] sps)
        {
        ...
        /// <param name="sps">參數列表</param>
        /// <returns></returns>
        public static bool Exists(CommandType type, string sql, OracleParameter[] sps)
        {
            DataTable dt = OracleHelper.GetDataTable(type, sql, sps);
            if (dt.Rows.Count > 0)
            {
                return true;
            }
        ...
        /// <param name="sps">參數列表</param>
        /// <returns></returns>
        public static bool Exists(string sql, OracleParameter[] sps)
        {
